...
public class EntityController : MonoBehaviour {
    ...
    private void AssignEntityDataStubs()
    {
        for (int i = 0; i < entities.Count; i++)
        {
            entities[i].InitializeData("Entities\\" + entities[i].Name + "_Prefab", "Portraits\\" + entities[i].Name, entities[i].AverageLifetime * 372.0f);            
        }
    }
    ...
    public void Breed(Entity primary, Entity secondary)
    {
        ...
        CreateEntity(ReturnEntityDataByName(chosenData.ResultingEntityName).EntityDataID, primary.transform.position, direction, primary.TerrainSegment, 5.0f);
    }
