...
public class SelectiveData {
    ...
    [SerializeField] private string primaryEntityName = "";
    public string PrimaryEntityName { get { return primaryEntityName; } }

    [SerializeField] private string secondaryEntityName = "";
    public string SecondaryEntityName { get { return secondaryEntityName; } }

    [SerializeField] private List<BreedingPartnerRequirement> primaryBreedingRequirements = new List<BreedingPartnerRequirement>();
    [SerializeField] private List<BreedingPartnerRequirement> secondaryBreedingRequirements = new List<BreedingPartnerRequirement>();       

    public bool Check(Entity entityA, Entity entityB)
    {
        ...
        if(entityA.GetData.Name == primaryEntityName && entityB.GetData.Name == secondaryEntityName)
        {
            primaryEntity = entityA;
            secondaryEntity = entityB;
        }
        else if (entityA.GetData.Name == secondaryEntityName && entityB.GetData.Name == primaryEntityName)
        {
        ...
        for (int i = 0; i < primaryBreedingRequirements.Count; i++)
        {
            Debug.Log("Checking Primary Requirements " + i);
            BreedingPartnerRequirement requirement = primaryBreedingRequirements[i];
    ...
    private bool CheckStringValue(BreedingPartnerRequirement requirement, Attribute attribute)
    {
        ...
        switch(requirement.AttributeOperator)
        {
            case BreedingPartnerRequirement.AttrOperator.GreaterThanOrEqual:
                Debug.LogError("Invalid Operator " + primaryEntityName + " + " + secondaryEntityName + ": Result = " + resultingEntityName);
                return false;
...
public class BreedingPartnerRequirement
{
    ...
    [SerializeField] private int partnerIndex = 0;   // 0 is owning entity, 1 is required entity mate
    public int PartnerIndex { get { return partnerIndex; } }
